...
Im obigen Test haben wir vier Variablen mit demselben Wert definiert. Jede Variable enthält einen Namen, der am besten zu dem Kontext passt, in dem sie verwendet werden soll. So passt beispielsweise "inputStudent" am besten in die Position der Eingabeparameter, während "storageStudent" am besten zu dem passt, was vom Storage Broker zurückgegeben wird, nachdem ein Schüler erfolgreich persistiert wurde.

Außerdem werden die Eingaben und Ausgaben in allen Tests nach dem Zufallsprinzip verteilt, um sicherzustellen, dass der Test auf ein bestimmtes Funktionsverhalten abzielt. Es gibt Fälle, in denen das Testen auf einen bestimmten Wert oder eine bestimmte Gruppe von Werten erforderlich ist. Aber dafür muss es eine gute Begründung geben.

Durch die Verwendung bestimmter Eingabeparameter oder Ausgabewerte kann man Tests leicht dazu bringen, einen erfolgreichen Vorgang zu melden. Randomisierung ist standardmäßig erforderlich, sofern nicht anders erforderlich. Wenn Sie zum Beispiel eine Funktion "Addieren (x, y)" haben und der Test speziell "1" und "2" an die Funktion übergibt und "3" als Rückgabewert erwartet, dann könnte jeder die eigentliche arithmetische Operation ignorieren und einfach immer "3" von der Zielfunktion zurückgeben. Betrachtet man dies auf einer größeren Unternehmensebene, wenn das Problem komplexer wird, wird deutlich, wie wichtig und entscheidend es ist, Ein- und Ausgaben standardmäßig zu randomisieren.
